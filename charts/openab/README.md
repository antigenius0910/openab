...
| `gateway.env` | Additional environment variables for the gateway container (map, `{name: value}`). Same shape as agent-level `env`. Useful for feature flags, `RUST_LOG` overrides, or HTTP proxy env vars if the gateway runs behind a transparent proxy. **Collision policy:** if a key here matches a chart-managed env name (`RUST_LOG`, `GATEWAY_WS_TOKEN`, `TELEGRAM_*`, `LINE_*`, `TEAMS_*`, `FEISHU_*`, `GOOGLE_CHAT_*`, `WECOM_*`), the built-in entry is omitted and only your value is rendered — Kubernetes uses `name` as the pod-env merge key, so emitting both would create ambiguity. | `{}` |
